...
void AI_Instant::mousePressEvent(QMouseEvent *event)
{
    ...
    if(ui.graphFrame->geometry().contains(coursePoint))
    {
        ...
        QColor color_get;
        QColor color_get_near1;
        QColor color_get_near2;
        QColor color_get_near3;
        QColor color_get_near4;

        int x = QCursor::pos().x();
        int y = QCursor::pos().y()-2;

        QList<QScreen *> list_screen =  QGuiApplication::screens(); //可能电脑接了多个屏幕
        QPixmap pixmap = list_screen.at(0)->grabWindow(0,x,y,1,5);
        if (!pixmap.isNull()) //如果像素图不为NULL
        {
            ...
            if (!image.isNull()) //如果image不为空
            {
                if (image.valid(0, 0)) //坐标位置有效
                {
                    ...
                    color_get_near1 = image.pixel(0,1);
                    color_get_near2 = image.pixel(0,2);
                    color_get_near3 = image.pixel(0,3);
                    color_get_near4 = image.pixel(0,4);
                }
            }
        }
        ...
        for(int i=0;i<configure.channelCount;i++)
        {
            ...
            if(color_get == SimpleGraph::lineColor[i] | color_get_near1==SimpleGraph::lineColor[i] | color_get_near2==SimpleGraph::lineColor[i] | color_get_near3==SimpleGraph::lineColor[i] | color_get_near4==SimpleGraph::lineColor[i])
                item->setText(str);
        }
    }
}
